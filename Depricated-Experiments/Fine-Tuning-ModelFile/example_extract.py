...
DEFAULT_CODES_CSV = "/spaces/25G05/61_codes.csv"
...
def build_scheme_map(codes_df: pd.DataFrame) -> Dict[str, Dict[str, List[str]]]:
    ...
    if not {'icd10_root', 'scheme_code', 'scheme_cause'}.issubset(set(tmp.columns)):
        raise ValueError("61_codes.csv must have columns: icd10_root, scheme_code, scheme_cause")

    tmp['icd10_root'] = tmp['icd10_root'].astype(str).str.strip().str.upper()
    tmp['scheme_code'] = tmp['scheme_code'].astype(str).str.strip()
    tmp['scheme_cause'] = tmp['scheme_cause'].astype(str).str.strip().str.strip('"')

    scheme_map: Dict[str, Dict[str, List[str]]] = {}
    for _, row in tmp.iterrows():
        sc = row['scheme_code']
        cause = row['scheme_cause']
        root = row['icd10_root']
        if sc not in scheme_map:
            scheme_map[sc] = {'scheme_cause': cause, 'roots': []}
        if not scheme_map[sc]['scheme_cause']:
            scheme_map[sc]['scheme_cause'] = cause
        if pd.notna(root) and root and root not in scheme_map[sc]['roots']:
            scheme_map[sc]['roots'].append(root)
    return scheme_map
...
def collect_examples_for_scheme(
    scheme_code: str,
    scheme_roots: List[str],
    ...
    work = clin_df.copy()
    work['icd10_prefix'] = work['ICD10Code'].apply(normalize_icd10_prefix)
    ...
    examples = []
    for _, r in matched.head(per_scheme).iterrows():
        icd = str(r.get('ICD10Code', ''))
        cause = str(r.get('CauseofDeath', ''))
        ind = str(r.get('individual_id', ''))
        pref = str(r.get('icd10_prefix', ''))
        examples.append((ind, icd, cause, pref))
    return examples

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--codes', default=DEFAULT_CODES_CSV, help='Path to 61_codes.csv')
    ap.add_argument('--va', default=DEFAULT_VA_CSV, help='Path to madiva_va_dataset_20250924.csv')
    ap.add_argument('--clin', default=DEFAULT_CLIN_CSV, help='Path to madiva_va_clinician_COD_20250926.csv')
    ap.add_argument('--out', default=DEFAULT_OUT_TXT, help='Output text file path')
    ap.add_argument('--per-scheme', type=int, default=DEFAULT_EXAMPLES_PER_SCHEME, help='Max examples per scheme code')
    args = ap.parse_args()

    codes_path = Path(args.codes)
    va_path = Path(args.va)
    clin_path = Path(args.clin)
    ...
    try:
        ...
        print(f"ERROR reading codes CSV: {codes_path} -> {e}", file=sys.stderr)
        sys.exit(1)

    try:
        va_df = pd.read_csv(va_path)
    except Exception as e:
        print(f"ERROR reading VA dataset CSV: {va_path} -> {e}", file=sys.stderr)
        sys.exit(1)
    ...
    if 'individual_id' not in va_df.columns:
        print("ERROR: VA dataset missing 'individual_id' column", file=sys.stderr)
        sys.exit(1)
    ...
    for scheme_code in sorted(scheme_map.keys()):
        scheme_cause = scheme_map[scheme_code]['scheme_cause']
        roots = scheme_map[scheme_code]['roots']

        examples = collect_examples_for_scheme(
        ...
        header = f" Scheme {scheme_code} — {scheme_cause}"
        lines.append(header)
        lines.append("-" * len(header))
        if not examples:
            lines.append(f"No matching cases found for scheme {scheme_code}.")
            lines.append("")
            continue
        ...
        for idx, (individual_id, icd, cause, matched_root) in enumerate(examples, start=1):
            ...
            if individual_id in va_index.index:
                row = va_index.loc[individual_id]
                prompt_text = format_case_for_llm(row)
                lines.append("EXAMPLE:")
                lines.append(prompt_text)
            else:
                lines.append(f"WARNING: individual_id {individual_id} not found in VA dataset.")
            lines.append("")  
            total_written += 1
    ...
    print(f"Wrote {total_written} example prompts across {len(scheme_map)} schemes to {out_path}")
    
    unique_ids = sorted(set(extracted_individual_ids))
    print("\nEXTRACTED_INDIVIDUAL_IDS_CSV:")
