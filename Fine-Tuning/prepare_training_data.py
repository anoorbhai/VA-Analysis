#!/usr/bin/env python3
"""
Data Preparation Script for Fine-Tuning Llama on Verbal Autopsy Data

This script prepares training and test datasets for fine-tuning by:
1. Loading VA questionnaire data and clinician COD labels
2. Mapping ICD-10 codes to the 61-cause scheme
3. Building instruction-style records with prompts and expected JSON outputs
4. Stratified splitting by scheme code to preserve cause distribution
5. Writing train.jsonl and test.jsonl files
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'prepare_training_data_{log_timestamp}.log'),
...
OUTPUT_DIR = Path("/home/noorbhaia/VA-Analysis/Fine-Tuning")
...
class DataPreparationPipeline:
    ...
    def clean_and_filter(self):
        ...
        # Extract ICD-10 root codes (first 3 characters) for mapping
    ...
    def merge_datasets(self):
        ...
        # Second merge: VA data with mapped COD
    ...
    def create_training_record(self, row: pd.Series) -> Dict:
        ...
        # Build the expected output (target)
        target = {
        ...
        # Create instruction-style record
        # Format suitable for fine-tuning: system + user prompt + assistant response
        record = {
            "messages": [
                {
                    "role": "system",
